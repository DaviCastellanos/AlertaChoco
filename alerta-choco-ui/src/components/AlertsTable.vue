<template>
  <div id="table" >
    <b-table responsive sticky-header="750px" striped hover light :items="this.alertItems" :fields="fields">
...
export default {
  ...
  data() {
      return {  
        fields: [ {tdClass: 'w-5', key:'primerMensaje', label: 'Primer mensaje'},
                  ...
                  {key: 'recibeLlamada', tdClass: 'w-1', sortable: true, label: 'Recibe llamada'},
                  {key: 'verificado', tdClass: 'w-1', sortable: true, label: 'Verificado'},
                  {key: 'completado', tdClass: 'w-1', sortable: true, label: 'Completado'},
                  {key: 'fechaReporte', tdClass: 'w-1', sortable: true, label: 'Fecha reporte'},
                  {key: 'puedeReportar', tdClass: 'w-1', sortable: true, label: 'Puede reportar'},
  ...
  methods: {
    ...
    GetAlertById (id) {
      return this.$store.getters.alerts.filter((obj)=>{ return obj.attributes.idAlerta === id; }).pop()
    }
  }, 
  computed: {
    alertItems() {
       ...
       if(features != undefined && features.length > 1) 
       {
        for(var i = 0 ; i < features.length; i++)
        {
          const date = new Date(features[i].attributes.fechaReporte)
            items.push({primerMensaje: this.FormatForHuman(features[i].attributes.primerMensaje), 
                        fechaReporte:  this.FormatForHuman(date.getDate() + "/" + (date.getMonth() + 1) + "/" + date.getFullYear()),
                        puedeReportar: this.FormatForHuman(features[i].attributes.puedeReportar),
                        ...
                        situacionActual: this.FormatForHuman(features[i].attributes.situacionActual),
                        recibeLlamada: this.FormatForHuman(features[i].attributes.recibeLlamada),
                        ...
                        botonVerificar: features[i].attributes.idAlerta
                        })
        }
       }
